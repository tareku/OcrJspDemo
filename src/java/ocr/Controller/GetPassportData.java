...
//                        spi.doPost(request, response);
    ...
    private PassportDetail setPassportDetail(String mrz, BufferedImage img,HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        p.setMrz(mrz.trim());         
        p.setDoctypecode(pm.getMrzDocTypeCode(firstline));
        p.setDoctypevalue(pm.getMrzDocTypeValue(p.getDoctypecode()));
        p.setStatecode(pm.getMrzStateCode(firstline));
        p.setStatevalue(pm.getMrzStateValue(p.getStatecode()));
        p.setType(pm.getMrzType(firstline));
        p.setNamecode(pm.getMrzNameCode(firstline));
        p.setNamevalue(pm.getMrzNameValue(p.getNamecode()));
        p.setPassnumbercode(pm.getMrzPassNumberCode(secondline));
        p.setPassnumbervalue(pm.getMrzPassNumberValue(secondline));
        p.setPassnumbercheckdigit(pm.getMrzPassNumberCheckDigit(secondline));
        p.setNationalitycode(pm.getMrzNationalityCode(secondline));
        p.setNationalityvalue(pm.getMrzStateValue(p.getNationalitycode()));
        p.setBirthdaycode(pm.getMrzBirthdayCode(secondline));
        p.setBirthdayvalue(pm.getMrzBirthdayValue(p.getBirthdaycode()));
        p.setBirthdaycheck(pm.getMrzBirthdayCkeck(secondline));
        p.setSex(pm.getMrzSexCode(secondline));
        p.setSexvalue(pm.getMrzSexValue(p.getSex()));
        p.setExpirationdatecode(pm.getMrzExpirationDateCode(secondline));
        p.setExpirationdatevalue(pm.getMrzExpirationDateValue(p.getExpirationdatecode()));
        p.setExpirationdatecheck(pm.getMrzExpirationDateCheck(secondline));
        p.setPersonalnumbercode(pm.getMrzPersonalNumberCode(secondline));
        p.setPersonalnumbervalue(pm.getMrzPersonalNumberValue(p.getPersonalnumbercode()));
